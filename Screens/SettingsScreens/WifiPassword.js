...
const WifiPassword= ({navigation}) => {
  ...
  const handleChangeWifiPassword = async () => {
    ...
    setLoading(true);
    try {
      const response = await axios.post('http://192.168.4.1/ChangeWifiPassword', {
        Password,
        ConfirmPassword,
      });

      if (response.data.success ) {
        console.log('Wifi Password Changed Succefully');
  ...
  return (
    ...
    <View>
      <View style={styles.TopLabel}>
        <Text style={styles.TopText}>Wifi Password</Text>
      </View>
      <View style={styles.FormContainer}>
        <Text style={styles.inputTitle}>Old Wifi Password</Text>
        ...
        <Text style={styles.inputTitle}>New Wifi Password</Text>
        ...
        <TouchableOpacity style={styles.FormSubmitBtn} onPress={handleChangeWifiPassword}>
        {loading ? (
